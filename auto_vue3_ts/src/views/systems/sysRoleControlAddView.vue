  ...
  <el-dialog
    ...
    <el-form :model="roleForm" :rules="roleRules" ref="roleFormRef">
      <el-form-item label="角色Code：" prop="roleCode">
        <el-input v-model="roleForm.roleCode" placeholder="角色Code" :maxlength="50" :show-word-limit="true" />
      </el-form-item>
      <el-form-item label="角色名称：" prop="roleName">
        <el-input v-model="roleForm.roleName" placeholder="角色名称" :maxlength="100" :show-word-limit="true" />
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button @click="dialogVisible = false">取消</el-button>
      <el-button type="primary" @click="saveRole(roleFormRef)">保存</el-button>
    </template>
  </el-dialog>
</template>
...
const openDialog = async (roleId?: string) => {
  ...
  if (roleId) {
    ...
    // 根据学校ID获取测试用例信息
    const getRoleByIdRes = await roleApi.getRoleById(roleId)
    // 获取查询出来的数据
    const data = getRoleByIdRes.data.data
...
const saveRole = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid) => {
    ...
    if (valid) {
      // 调用后端测试用例信息保存接口
      const res = await roleApi.save(roleForm.value)
      // 判断是否保存成功
      if (res.data.success) {
        // 提示保存成功
